...
I2CRaiseSCL(I2CBusPtr b, int sda, int timeout)
{
    ...
    if (i <= 0) {
	I2C_TIMEOUT(LOG("[I2CRaiseSCL(<%s>, %d, %d) timeout]", b->BusName, sda, timeout));    
	return FALSE;
    }
...
I2CPutByte(I2CDevPtr d, I2CByte data)
{
	...
	if (i <= 0) {
	    I2C_TIMEOUT(LOG("[I2CPutByte(<%s>, 0x%02x, %d, %d, %d) timeout]", 
				       b->BusName, data, d->BitTimeout, 
				       d->ByteTimeout, d->AcknTimeout));
	    r = FALSE;
	}

	I2C_TRACE(LOG("W%02x%c ", (int) data, sda ? '-' : '+'));
    }
...
I2CGetByte(I2CDevPtr d, I2CByte *data, Bool last)
{
    ...
    *data = (sda > 0) << 7;
    ...
    I2C_TRACE(LOG("R%02x%c ", (int) *data, last ? '+' : '-'));
...
{
	I2CDevPtr tmp;
	tmp = IONew(I2CDevRec, 1);
	if (!tmp) return NULL;
	bzero(tmp, sizeof(I2CDevRec));
    return tmp;
}
...
xf86DestroyI2CDevRec(I2CDevPtr d, Bool unalloc) 
{
	...
	LOG("I2C device \"%s:%s\" removed.\n",
		   d->pI2CBus->BusName, d->DevName);

	if (unalloc) IODelete(d, I2CDevRec, 1);
    }
}
...
xf86I2CDevInit(I2CDevPtr d) 
{
    ...
    LOG("I2C device \"%s:%s\" registered at address 0x%02X.\n",
...
xf86DestroyI2CBusRec(I2CBusPtr b, Bool unalloc, Bool devs_too) 
{
	...
	if (b->FirstDev != NULL) {
		...
		if (unalloc) {
		    LOG("i2c bug: Attempt to remove I2C bus \"%s\", "
	...
	LOG("I2C bus \"%s\" removed.\n",
		   b->BusName);

	if (unalloc) IODelete(b, I2CBusRec, 1);
    }
}
...
xf86I2CGetScreenBuses(int scrnIndex, I2CBusPtr **pppI2CBus)
{
	...
	*pppI2CBus = IONew(I2CBusPtr, n);
	if (!(*pppI2CBus)) return 0;
	bzero(*pppI2CBus, sizeof(I2CBusPtr) * n);
